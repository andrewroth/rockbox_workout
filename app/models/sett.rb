class Sett < ActiveRecord::Base
  ...
  belongs_to :exercise
  has_many :functions, :foreign_key => "set_id"

  #has_many :set_log_entries, :foreign_key => "set_log_entry_id"  # ???

  def self.write_csv
    super do
      WorkoutDate.where("finished_at IS NULL").collect(&:workout).flatten.collect(&:exercises).flatten.collect(&:sets).flatten.uniq
    end 
  end 
